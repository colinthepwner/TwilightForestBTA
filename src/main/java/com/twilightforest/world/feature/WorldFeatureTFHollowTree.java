...
public class WorldFeatureTFHollowTree extends TFWorldFeature {
	...
	protected void buildBranchRing(int branchHeight, int heightVar, int length, int lengthVar,
		...
		for (int dx = -this.diameter; dx <= this.diameter; dx++) {
			for (int dz = -this.diameter; dz <= this.diameter; dz++) {
				for (int dy = -4; dy < 0; dy++) {
					...
					int block = this.hasAirAround(this.worldObj, dx + this.x, dy + this.y, dz + this.z)
						? this.treeBlock
						: this.rootBlock;
					this.putBlock(dx + this.x, dy + this.y, dz + this.z, block, true);
				}
			}
		}
	...
	private void makeRoot(int branchHeight, double length, double angle, double tilt) {
		int[] src = this.translate(this.x, this.y + branchHeight, this.z, this.diameter, angle, 0.5);
		int[] dest = this.translate(src[0], src[1], src[2], length, angle, tilt);
		int[] line = getBresehnamArray(src[0], src[1], src[2], dest[0], dest[1], dest[2]);
		...
		for (int i = 0; i < line.length; i += 3) {
			...
			int block;
			if (stillAboveGround && this.hasAirAround(this.worldObj, px, py, pz)) {
				block = this.treeBlock;
			} else {
				block = this.rootBlock;
				stillAboveGround = false;
			}

			this.putBlock(px, py, pz, block, true);
			this.putBlock(px, py - 1, pz, block, true);
		}
	}
	...
	private void addFirefly(int fHeight, double fAngle) {
		this.putCritter(fHeight, fAngle, TFBlocks.FIREFLY);
	}

	private void addCicada(int fHeight, double fAngle) {
		this.putCritter(fHeight, fAngle, TFBlocks.CICADA);
	}

	private void putCritter(int fHeight, double fAngle, net.minecraft.core.block.Block<?> critter) {
		...
		if (critter.canPlaceAt(this.worldObj, new TilePos(src[0], src[1], src[2]))) {
			this.putBlockAndMetadata(src[0], src[1], src[2], critter.id(), meta, false);
		}
	}
