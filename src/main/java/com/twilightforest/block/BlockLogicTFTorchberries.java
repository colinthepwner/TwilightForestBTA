...
public class BlockLogicTFTorchberries extends BlockLogic {

	public BlockLogicTFTorchberries(@NotNull Block<?> block) {
	...
	public static boolean canPlaceRootBelow(@NotNull World world, int x, int y, int z) {
		Material above = world.getBlockMaterial(new TilePos(x, y, z));
		if (above == Materials.DIRT || above == Materials.GRASS) {
			return true;
		}
		return world.getBlockId(x, y, z) == TFBlocks.ROOTS.id();
	}
